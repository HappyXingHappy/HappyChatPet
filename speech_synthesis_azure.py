...
def speech_synthesis_with_voice():
    ...
    while True:
        print("Enter some text that you want to speak, Ctrl-Z to exit")
        try:
            # use SSML to change style
            text = input() 
            # ssml_string = '<speak version="1.0" xmlns="https://www.w3.org/2001/10/synthesis" xml:lang="zh-CN">'
            # ssml_string = ssml_string = + '<voice name="zh-CN-XiaoxuanNeural"><s /><mstts:express-as role="Girl" style="lyrical">'
            # ssml_string = ssml_string + input() + '</mstts:express-as><s /></voice></speak>'
            ssml_string = """<speak version='1.0' xml:lang='{}' xmlns='http://www.w3.org/2001/10/synthesis' xmlns:mstts='http://www.w3.org/2001/mstts' xmlns:emo='http://www.w3.org/2009/10/emotionml' >
                <voice name='{}'>
                    <mstts:viseme type='FacialExpression'/>
                    <mstts:express-as style='{}'/>
                    {}
                </voice>
            </speak>""".format(language,voice,style,text)
            # print(ssml_string)
        except EOFError:
            break
        # result = speech_synthesizer.speak_text_async(text).get()
...
def speech_synthesis_with_voice(rstext):
    """performs speech synthesis to the default speaker with specified voice"""
    # Creates an instance of a speech config with specified subscription key and service region.
    speech_config = speechsdk.SpeechConfig(subscription=speech_key, region=service_region)
    # Sets the synthesis voice name.
